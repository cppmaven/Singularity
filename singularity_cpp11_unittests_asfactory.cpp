...
BOOST_AUTO_TEST_CASE(passOneArgumentByValue) {
    ...
    Horizon & horizon = singularity<Horizon, single_threaded>::create(value);

    (void)horizon;
    Horizon & horizonGlobal = singularity<Horizon, single_threaded>::get();
    (void)horizonGlobal;
...
BOOST_AUTO_TEST_CASE(passOneArgumentByAddress) {
    Event event;
    Horizon & horizon = singularity<Horizon, single_threaded>::create(&event);
    (void)horizon;
    singularity<Horizon, single_threaded>::destroy();
}
...
BOOST_AUTO_TEST_CASE(passThreeArguments) {
    int value = 3;
    Event event;
    typedef singularity<Horizon, single_threaded> singularityType;
...
BOOST_AUTO_TEST_CASE(shouldThrowOnDoubleCallsWithDifferentArguments) {
    ...
    int value = 5;
    BOOST_CHECK_THROW( // Call create() twice in a row
        Horizon & horizon2 = (singularity<Horizon, single_threaded>::create(value)),
        boost::singularity_already_created
    );
